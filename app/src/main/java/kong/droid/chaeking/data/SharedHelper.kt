package kong.droid.chaeking.data
...
class SharedHelper(val context: Context) {
    ...
    var sharedPreferences: SharedPreferences =
        context.getSharedPreferences(Constants.SHARED, Context.MODE_PRIVATE)
    ...
    val getFbToken: String
        get() = sharedPreferences.getString(Constants.Shared.FB_TOKEN, "") ?: ""

    val getAccessToken: String
        get() = sharedPreferences.getString(Constants.Shared.ACCESS_TOKEN, "") ?: ""

    val getRefreshToken: String
        get() = sharedPreferences.getString(Constants.Shared.REFRESH_TOKEN, "") ?: ""

    val getUserNick: String
        get() = sharedPreferences.getString(Constants.Shared.USER_NICK, "") ?: ""

    val getUserEmail: String
        get() = sharedPreferences.getString(Constants.Shared.USER_EMAIL, "") ?: ""

    val getUserProfile: String
        get() = sharedPreferences.getString(Constants.Shared.USER_PROFILE, "") ?: ""
    ...
    private fun newEncryptUtil(mode: Int, input: ByteArray): String {
        return try {
            val key = (Constants.CHAEKING + getFbToken).substring(0, 32).toByteArray(Charset.defaultCharset())
